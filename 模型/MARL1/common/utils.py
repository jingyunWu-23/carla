...
class VideoRecorder:
    def __init__(self, filename, frame_size, fps=5):
        ...
        try:
            import imageio
            self.writer = imageio.get_writer(filename, fps=fps, format='FFMPEG', codec='libx264', quality=8)
        except ImportError:
...
def init_dir(base_dir):
    dirs = {
        ...
        'train_videos': os.path.join(base_dir, 'train_videos'),
        'eval_videos': os.path.join(base_dir, 'eval_videos')
    }
...
def copy_file_ppo(config_dir):
    src = 'configs/configs_ppo.ini'
    dst = os.path.join(config_dir, 'configs_ppo.ini')
    if os.path.exists(src):
        shutil.copy2(src, dst)
        print(f"Copied {src} to {dst}")


def copy_file_akctr(config_dir):
    src = 'configs/configs_acktr.ini'
    dst = os.path.join(config_dir, 'configs_acktr.ini')
    if os.path.exists(src):
        shutil.copy2(src, dst)
        print(f"Copied {src} to {dst}")
