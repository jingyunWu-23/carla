...
config_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'configs', 'configs_ppo.ini')
config = configparser.ConfigParser()
if not config.read(config_dir):
    raise FileNotFoundError("MAPPO config file was not found: {}".format(config_dir))
...
class MAPPO:
    ...
    """
    def __init__(self, env, state_dim, action_dim,                        #主要的超参数设置在config——ppo文件里
                 memory_capacity=10000, max_steps=None,
                 roll_out_n_steps=1, target_tau=1.,
                 target_update_steps=5, clip_param=0.2,
                 reward_gamma=0.99, reward_scale=100,
                 actor_hidden_size=128, critic_hidden_size=128,
                 actor_output_act=nn.functional.log_softmax, critic_loss="mse",
                 actor_lr=0.001, critic_lr=0.001, test_seeds=0,
                 optimizer_type="adam", entropy_reg=0.01,
                 max_grad_norm=0.5, batch_size=240, episodes_before_train=100,
        ...
        self.env_state,_ ,self.obs2,self.obs3= self.env.reset()   #从这里环境开始启动，产生三个state，分别是MAPPO控制的三辆车，还有其他两种PPO模型控制的两辆车
        self.n_episodes = 0
        self.n_steps = 0
    ...
    def interact(self):
        if (self.max_steps is not None) and (self.n_steps >= self.max_steps):
            self.env_state,_,self.obs2,self.obs3= self.env.reset()
            self.n_steps = 0
        ...
        for i in range(self.roll_out_n_steps):
            ...
            action = self.exploration_action(self.env_state, self.n_agents)  #actor模型从这里输出三个车的动作

            model = PPO.load("PPOmodel2")   #这里的PPOmodel模型是训练好的模型，你输入state他就能输出action
            # self.obs2=[[ 1.    ,      1.    ,      0.        ,  0.41666667 , 0.        ], [ 1.  ,        0.14457707 , 0.66666667, -0.06090748,  0.        ], [ 1.     ,     0.30139512 , 0.     ,    -0.05462352 , 0.        ], [ 1.       ,   0.44832594 , 0.   ,      -0.02885331 , 0.        ], [ 1,0.6,0,-0.27,0]]

            action2,_ =model.predict(self.obs2)
            model1 = PPO.load("PPOmodel889")
            action3,_ = model1.predict(self.obs3)


            action.append(action2)
            action.append(action3)

            # print('wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww')
            next_state, global_reward, done, info,self.obs2 ,self.obs3= self.env.step(tuple(action))
            # self.env.render(mode='human')
            actions.append([index_to_one_hot(a, self.action_dim) for a in action[:self.n_agents]])
    ...
    def train(self):
        ...
        for agent_id in range(self.n_agents):
            ...
            values = self.critic_target(states_var
                                        [:, agent_id, :], actions_var[:, agent_id, :]).detach()
    ...
    def evaluation(self, env, output_dir, eval_episodes=1, is_train=True):
        print('执行了evaluation33333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333')
        rewards = []
        infos = []
        avg_speeds = []
        steps = []
        ...
        for i in range(eval_episodes):
            ...
            #因为加了一个obs所以参数变多，这里我改了，加了一个_
            if is_train:
                if self.traffic_density == 1:
                    state, action_mask,self.obs2,self.obs3 = env.reset(is_training=False, testing_seeds=seeds[i], num_CAV=i + 1)           #产生三个参数最后一个是obs2
                elif self.traffic_density == 2:
                    state, action_mask,_ = env.reset(is_training=False, testing_seeds=seeds[i], num_CAV=i + 2)
                elif self.traffic_density == 3:
                    state, action_mask,_ = env.reset(is_training=False, testing_seeds=seeds[i], num_CAV=i + 4)
            else:
                state, action_mask,self.obs2 ,self.obs3= env.reset(is_training=False, testing_seeds=seeds[i])
            ...
            while not done:
                ...
                #我加的
                model = PPO.load("PPOmodel2.zip")
                # model = A2C.load("a2c_highway")
                # model = DQN.load("highway_dqn_model")
                action2, _ = model.predict(self.obs2)
                action.append(action2)
                model1 = PPO.load("PPOmodel889.zip")
                action3,_ = model1.predict(self.obs3)

                action.append(action3)
                state, reward, done, info,self.obs2,self.obs3 = env.step(action)
            ...
            collision_episodes.append(episode_collision)

        if video_recorder is not None:
            video_recorder.release()
        env.close()
        return rewards, (vehicle_speed, vehicle_position), steps, avg_speeds, collision_episodes
