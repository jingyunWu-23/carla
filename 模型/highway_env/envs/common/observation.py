...
class MultiAgentObservation(ObservationType):
    def __init__(self,
        ...
        obs_type = observation_factory(self.env, self.observation_config)
        obs_type.observer_vehicle = self.env.non_vehicle
        self.agents_observation_types.append(obs_type)

        hdv_vehicles = getattr(self.env, "hdv_vehicles", None) or []
        if not hdv_vehicles and getattr(self.env, "ziran_vehicle", None) is not None:
            hdv_vehicles = [self.env.ziran_vehicle]
        for hdv_vehicle in hdv_vehicles:
            obs_type = observation_factory(self.env, self.observation_config)
            obs_type.observer_vehicle = hdv_vehicle
            self.agents_observation_types.append(obs_type)
