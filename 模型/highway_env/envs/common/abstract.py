...
class AbstractEnv(gym.Env):
    ...
    def vehicle(self, vehicle: Vehicle) -> None:
        ...
        self.non_vehicle=[vehicle]
        self.ziran_vehicle = [vehicle]
    ...
    def reset(self, is_training=True, testing_seeds=0, num_CAV=0, num_HDV=0) -> Observation:
        """
        ...
        self.define_spaces()  # First, to set the controlled vehicle class depending on action space璁剧疆瑙傚療绌洪棿鍜岃溅杈嗗姩浣滅┖闂寸殑绫诲瀷
        self.time = self.steps = 0
        self.seed += 1
        self.done = False
        self.vehicle_speed = []
        self.vehicle_pos = []
        self._reset(num_CAV=num_CAV, num_HDV=num_HDV)           #瀵硅矾鍜岃溅杩涜璁剧疆
        self.define_spaces()  # Second, to link the obs and actions to the vehicles once the scene is created
        # set the vehicle id for visualizing
        for i, v in enumerate(self.road.vehicles):
            v.id = i
        obs1 = self.observation_type.observe()

        n_controlled = len(self.controlled_vehicles)
        obs = obs1[:n_controlled]
        obs2 = obs1[n_controlled]
        self.obs_hdv_list = list(obs1[n_controlled + 1:])
        obs3 = self.obs_hdv_list[0] if self.obs_hdv_list else np.zeros_like(obs2)
        # get action masks
        ...
        return (
            np.asarray(obs).reshape((len(obs), -1)),
            np.array(available_actions),
            np.asarray(obs2).reshape((len(obs2), -1)),
            np.asarray(obs3).reshape((len(obs2), -1)),
        )

    def _reset(self, num_CAV=1, num_HDV=0) -> None:
        """
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:
        ...
        # print(self.steps)
        return obs, reward, terminal, info,obs2,obs3
    ...
    def _simulate(self, action: Optional[Action] = None) -> None:
        ...
        for _ in range(int(self.config["simulation_frequency"] // self.config["policy_frequency"])):
            ...
            self.road.act(action)  # Execute an action
    ...
    def _compute_headway_distance(self, vehicle ):
        ...
        for v in self.controlled_vehicles:
            ...
            if (vehicle.lane_index != ("b", "c", 1)) and (
                ...
                if hd < headway_distance:
                    headway_distance = hd
        # a=self.road.vehicles[3]               #绠楀嚭鍏朵粬杞﹁窛绂婚潪浠ｇ悊杞︾殑璺濈锛岃鍏跺畠杞﹁窛绂婚潪浠ｇ悊杞﹁秺杩戣秺濂?        # hd1 = a.position[0] - vehicle.position[0]
        # hd1=abs(hd1)
        # nonagent_distance=hd1

        return headway_distance
    # def _compute_headway_distance1(self, vehicle ):
    #     a=self.road.vehicles[3]               #绠楀嚭鍏朵粬杞﹁窛绂婚潪浠ｇ悊杞︾殑璺濈锛岃鍏跺畠杞﹁窛绂婚潪浠ｇ悊杞﹁秺杩戣秺濂?    #     hd1 = a.position[0] - vehicle.position[0]
    #     hd1=abs(hd1)
    #     nonagent_distance=hd1
    #
    #     return nonagent_distance
    def _compute_headway_distance1(self, vehicle ):
        a = getattr(self, "ego_vehicle", None)
        if a is None:
            ego_index = len(getattr(self, "controlled_vehicles", []))
            if len(self.road.vehicles) <= ego_index:
                return 60
            a = self.road.vehicles[ego_index]
        distance = math.sqrt(pow(a.position[0] - vehicle.position[0], 2) + pow(a.position[1] - vehicle.position[1], 2))
