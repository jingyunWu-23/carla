...
class Road(object):
    ...
    def act(self, a) -> None:
        ...
        if isinstance(a, dict):
            adv_actions = list(a.get("adv_cav") or [])
            ego_action = a.get("ego", 1)
            hdv_actions = list(a.get("hdv") or [])
        else:
            actions = list(a) if a is not None else []
            n_adv = sum(1 for v in self.vehicles if getattr(v, "role", None) == "adv_cav")
            adv_actions = actions[:n_adv]
            ego_action = actions[n_adv] if len(actions) > n_adv else 1
            hdv_actions = actions[n_adv + 1:] if len(actions) > n_adv + 1 else []

        hdv_index = 0
        for vehicle in self.vehicles:
            role = getattr(vehicle, "role", None)
            if role == "adv_cav":
                # MultiAgentAction already applied these actions to controlled vehicles.
                continue
            if role == "ego":
                vehicle.act(ego_action)
            elif role == "hdv":
                action = hdv_actions[hdv_index] if hdv_index < len(hdv_actions) else (
                    hdv_actions[-1] if hdv_actions else 1
                )
                vehicle.act(action)
                hdv_index += 1
            else:
