...
class ScenarioManager:
    ...
    def _connect(self):
        try:
            ...
            raise RuntimeError(
                ...
                "Install carla==0.9.15 or use backend='mock' for interface tests."
            ) from exc
    ...
    def _compute_route_length(self, scenario_config: Dict[str, Any]) -> float:
        return float(scenario_config.get("route_length", self.config.get("route_length", 500.0)))
    ...
    def _reset_mock(self, scenario_config: Dict[str, Any], seed: Optional[int]):
        ...
        num_cav = int(scenario_config.get("num_cav", 1))
        num_hdv = int(scenario_config.get("num_hdv", 3))
        route_length = float(scenario_config.get("route_length", 500.0))
    ...
    def _update_metrics(self):
        ...
        if ego is not None:
            self.state.route_completion = float(np.clip(ego.route_s / max(self.state.route_length, 1e-6), 0.0, 1.0))
            self.state.success = self.state.route_completion >= 1.0 and not ego.crashed
        else:
            self.state.route_completion = 0.0
            self.state.success = False
